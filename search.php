...
if ($_GET['search'] == '') {
  ...
  $counts = $db->query("SELECT COUNT(*) AS cnt FROM posts WHERE title LIKE '%" . $_GET["search"] . "%' ");
  $cnt = $counts->fetch();
  
  $posts = $db->query("SELECT p.*, u.nickname FROM posts p, users u WHERE p.user_id=u.id AND title LIKE '%" . $_GET["search"] . "%' ORDER BY p.created DESC");  
}
